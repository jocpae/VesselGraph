...
def train(model, predictor, data, split_edge, optimizer, batch_size,splitting_strategy):
    ...
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size,
        ...
        print(pos_out)
        pos_loss = -torch.log(pos_out + 1e-15).mean()*10
        #pos_loss = -torch.log(pos_out).mean()*10
        #print(pos_loss)
        if splitting_strategy == 'random':
            ...
            neg_out = predictor(h[edge[0]], h[edge[1]])
            neg_loss = -torch.log(1 - neg_out + 1e-15).mean()*10
...
def main():
    ...
    for i in range(data.edge_index.shape[1]):
        edge1 = int(data.edge_index[0, i])
        edge2 = int(data.edge_index[1, i])
        liste.append([[data.x[edge1, 0], data.x[edge1, 1], data.x[edge1, 2]],
                      [data.x[edge2, 0], data.x[edge2, 1], data.x[edge2, 2]]])
    print(liste)
    '''
    #data.x[:, 0] = torch.nn.functional.normalize(data.x[:, 0], dim=0)
    #data.x[:, 1] = torch.nn.functional.normalize(data.x[:, 1], dim=0)
    #data.x[:, 2] = torch.nn.functional.normalize(data.x[:, 2], dim=0)
    #data.x[:, 3] = torch.nn.functional.normalize(data.x[:, 3], dim=0)
    '''
    liste = []
    for i in range(data.edge_index.shape[1]):
        edge1 = int(data.edge_index[0,i])
            ...
            if epoch % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    if valid_res > best_val:
                        best_val = valid_res
                        torch.save(model.state_dict(), os.path.join(args.log_dir,"model_dict"))
