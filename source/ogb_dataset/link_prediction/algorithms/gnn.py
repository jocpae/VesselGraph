...
def main():
    ...
    parser.add_argument('--dataset', type=str,default='ogbl-italo_random_no_edge_feat')
    parser.add_argument('--splitting_strategy',type=str,default='random')
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    writer.add_scalar("Accuracy/train", train_res, epoch)
                    writer.add_scalar("Accuracy/valid", valid_res, epoch)
                    writer.add_scalar("Accuracy/test", test_res, epoch)
                    writer.add_scalars("Accuracy/Summary", {'train_acc': train_res, 'valid_acc': valid_res,
                                                            'test_acc': test_res}, epoch)
        ...
        #writer.add_hparams(
        #    vars(args),
        #    {'hparam/accuracy': best_val_acc, 'hparam/loss': loss})
        writer.flush()
        writer.close()
    print('GraphSAGE' if args.use_sage else 'GCN')
