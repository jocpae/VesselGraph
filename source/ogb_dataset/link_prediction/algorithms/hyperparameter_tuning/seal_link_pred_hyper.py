...
sys.path.insert(0, '../../../../pytorch_dataset')
...
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 ...
                 max_nodes_per_hop=None, directed=False, splitting_strategy='seal_random', use_edge_feature=False):
        self.data = data
...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 use_coalesce=False, node_label='drnl', ratio_per_hop=1.0, 
                 max_nodes_per_hop=None, directed=False, splitting_strategy='seal_random', **kwargs):
        self.data = data
        self.split_edge = split_edge
        self.num_hops = num_hops
...
if not args.keep_old:
    # Backup python files.
    copy('seal_link_pred_hyper.py', args.res_dir)
...
if args.dataset.startswith('ogbl'):
    ...
    if args.use_edge_attr:
        data.edge_weight = torch.absolute(data.edge_attr[:, 2])  # radius

    #from sklearn.preprocessing import MinMaxScaler

    #scaler = MinMaxScaler()
    #weights = data.edge_weight.numpy().reshape(-1,1)
    #scaler.fit(weights)
    #weights = scaler.transform(weights)
    #data.edge_weight = torch.tensor(1+weights)
    print("test")
