...
hparam_dict = {
    'lr': [2.e-05, 1.e-4, 1.e-03, 3.e-3, 1.e-2],  # np.logspace(-6, -3, 10),
    'num_layers': [2, 3, 4],
...
scripts_started = 0
with open('parallelize_mlp.sh', 'w+') as f:
    ...
    for i in range(comb):
        ...
        f.write(f'CUDA_VISIBLE_DEVICES={available_gpus[i % len(available_gpus)]} python mlp.py {param_str} &\n')
